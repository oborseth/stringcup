...
| **Rendezvous tokens are bearer secrets** | Whoever holds one can claim a role in that pairing. Server-issued so entropy is guaranteed, single-claim so theft is detectable (409), and 30-minute-lived (raised from 15: the documented flow is a human pasting to a second agent that may need installing and a restart) — but interception in transit is not preventable |
